...
def forward_transform(
    ...
    r"""Compute the forward adjoint directional wavelet transform on the ball with Numpy.
    ...
    f_scal_lmp = laguerre.forward_transform(f_scal, leg_kernels[1], lag_polys[1])
    ...
    for jp in range(Jp + 1):
        for jl in range(Jl + 1):
            ...
            f_wav_lmnp[jp][jl] = (
                wigner_laguerre.forward_transform(
                    f_wav[jp][jl],
                    wig_kernels[jl],
                    wav_lag_polys[jp],
                    L_l,
            ...
            flmp[:L_p, :L_l, L - L_l : L - 1 + L_l] += np.einsum(
                "pnlm, pln->plm",
                f_wav_lmnp[jp][jl][:, ::2, :, :],
                np.conj(wav_lmp[jp][jl][:, :, L_l - Nj : L_l - 1 + Nj : 2]),
            )

    return laguerre.inverse_transform(flmp, leg_kernels[0], lag_polys[0])
...
def forward_transform_jax(
    ...
    r"""Compute the forward adjoint directional wavelet transform on the ball with JAX and JIT.
    ...
    f_scal_lmp = laguerre.forward_transform_jax(f_scal, leg_kernels[1], lag_polys[1])
    ...
    for jp in range(Jp + 1):
        for jl in range(Jl + 1):
            L_l, L_p, Nj = wavelet_scale_limits_N(L, P, N, jl, jp, lam_l, lam_p)
            f_wav_lmnp[jp][jl] = (
                wigner_laguerre.forward_transform_jax(
                    f_wav[jp][jl],
                    wig_kernels[jl],
                    wav_lag_polys[jp],
                    L_l,
            ...
            flmp = flmp.at[:L_p, :L_l, L - L_l : L - 1 + L_l].add(
                jnp.einsum(
                    "pnlm, pln->plm",
                    f_wav_lmnp[jp][jl][:, ::2, :, :],
                    jnp.conj(wav_lmp[jp][jl][:, :, L_l - Nj : L_l - 1 + Nj : 2]),
                    optimize=True,
                )
            )

    return laguerre.inverse_transform_jax(flmp, leg_kernels[0], lag_polys[0])
...
def inverse(
    ...
    r"""Compute the inverse adjoint directional wavelet transform on the ball.
...
def inverse_transform(
    ...
    r"""Compute the inverse adjoint directional wavelet transform on the ball with Numpy.
    ...
    Jl = j_max(L, lam_l)
    Jp = j_max(P, lam_p)
    flmp = laguerre.forward_transform(f, leg_kernels[1], lag_polys[1])

    # Compute scaling coefficients
    f_scal_lmp = np.einsum("plm,pl->plm", flmp, scal_lmp)
    f_scal = laguerre.inverse_transform(f_scal_lmp, leg_kernels[0], lag_polys[0])
    ...
    for jp in range(Jp + 1):
        for jl in range(Jl + 1):
            ...
            f_wav_lmnp[jp][jl] = (
                wigner_laguerre.inverse_transform(
...
def inverse_transform_jax(
    ...
    r"""Compute the inverse adjoint directional wavelet transform on the ball with JAX and JIT.
    ...
    Jl = j_max(L, lam_l)
    Jp = j_max(P, lam_p)
    flmp = laguerre.forward_transform_jax(f, leg_kernels[1], lag_polys[1])

    # Compute scaling coefficients
    f_scal_lmp = jnp.einsum("plm,pl->plm", flmp, scal_lmp, optimize=True)
    f_scal = laguerre.inverse_transform_jax(f_scal_lmp, leg_kernels[0], lag_polys[0])
    ...
    for jp in range(Jp + 1):
        for jl in range(Jl + 1):
            ...
            f_wav_lmnp[jp][jl] = (
                wigner_laguerre.inverse_transform_jax(
                    f_wav_lmnp[jp][jl], wig_kernels[jl], wav_lag_polys[jp], L_l
                )
                * 2
                * jnp.pi
