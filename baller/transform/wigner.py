...
def forward(
    ...
    if wigner_kernel is None:
        kernel = baller.construct.wigner_constructor.load_wigner_matrix(
...
def forward_transform(
    f: np.ndarray, kernel: np.ndarray, L: int, N: int
) -> np.ndarray:
...
def inverse(
    ...
    if method == "numpy":
        ...
        return (
            forward_transform_jax(fnlm, kernel, L, N)
            * (2 * N - 1)
            / (2 * jnp.pi)
            if adjoint
...
def inverse_transform(
    fnlm: np.ndarray, kernel: np.ndarray, L: int
) -> np.ndarray:
    r"""Compute the forward Wigner transform with Numpy.
...
def inverse_transform_jax(
    ...
    return jnp.fft.ifft(
        jnp.fft.ifftshift(fnab, axes=-3), norm="forward", axis=-3
    )
