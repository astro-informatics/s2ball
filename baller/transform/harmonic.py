...
def forward(
    ...
    if legendre_kernel is None:
        kernel = baller.construct.legendre_constructor.load_legendre_matrix(
    ...
    if method == "numpy":
        return (
            inverse_transform(f, kernel)
            if adjoint
            else forward_transform(f, kernel)
        )
    elif method == "jax":
        return (
...
def forward_transform_jax(
    f: jnp.ndarray, legendre_kernel: jnp.ndarray
) -> jnp.ndarray:
...
def inverse_transform_jax(
    ...
    fm = jnp.einsum(
        "...lmi, ...lm->...im", legendre_kernel, flm_shift, optimize=True
    )
