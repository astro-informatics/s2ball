...
def test_flm_reindexing_functions(flm_generator):
    ...
    flm_1d = baller.utils.flm_2d_to_1d(flm_2d, L)

    assert len(flm_1d.shape) == 1

    flm_2d_check = baller.utils.flm_1d_to_2d(flm_1d, L)
...
def test_flm_reindexing_exceptions(flm_generator):
    ...
    with pytest.raises(ValueError) as e:
        baller.utils.flm_2d_to_1d(flm_1d, L)

    with pytest.raises(ValueError) as e:
        baller.utils.flm_2d_to_1d(flm_3d, L)

    with pytest.raises(ValueError) as e:
        baller.utils.flm_1d_to_2d(flm_2d, L)

    with pytest.raises(ValueError) as e:
        baller.utils.flm_1d_to_2d(flm_3d, L)
