...
Installation
=========================
There are two primary ways to install ``S2BALL``. One can either build the project from 
...
Quick install (PyPi)
--------------------
The simplest way to pick up ``S2BALL`` is to install it directly from PyPi by running 

.. code-block:: bash
    
    pip install s2ball 

after which ``S2BALL`` may be imported and run as outlined in the associated notebooks and collab tutorials.
...
Once within a fresh environment ``S2BALL`` may be installed by cloning the GitHub repository 
and pip installing locally

.. code-block:: bash

    git clone https://github.com/astro-informatics/s2ball
    cd s2ball
    pip install .
